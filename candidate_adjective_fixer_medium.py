...
def add_adj_candidate(session, BiomarkerMedium, candidates, dSplit):
    needsAdding = []
    justAdded = []
    for c in candidates:
        ...
        if (len(to_append_string) > 0):
            ...
            NewCandiateStable = "%s::%s:%s:%s" % (temp_splits[0], "span", (c.medium.char_start - len(to_append_string)), c.medium.char_end)
            ...
            ConditionSpan = Span(stable_id=NewCandiateStable,
                                 ...
                                 char_start=(c.medium.char_start - len(to_append_string)),
            ...
            query = session.query(BiomarkerMedium).filter(BiomarkerMedium.biomarker_id == BiomarkerSpan.id)
